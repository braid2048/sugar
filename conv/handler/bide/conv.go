...
func (h *Handler) DoConv(ctx context.Context, req *types.ConvReq) (*types.ConvRes, error) {
	...
	if err := h.Validate(req); err != nil {
		return nil, fmt.Errorf("bide-conv -- Validate err: %w", err)
	}
	// step2. 构造参数
	convReq, err := h.MakeReq(req)
	if err != nil {
		return nil, fmt.Errorf("bide-conv -- MakeReq err: %w", err)
	}

	// step3. 回传
	respCode, respBody, err := utils.SendGetRequest(ctx, convReq.Req, nil)
	if err != nil {
		return nil, fmt.Errorf("bide-conv -- SendGetRequest err: %w", err)
	}
...
func (h *Handler) Validate(req *types.ConvReq) error {
	...
	// NOTE: 必填字段
	if req.BideParams == nil {
		return fmt.Errorf("conv params is nil")
	}
	// NOTE: callback
	if req.BideParams.Callback == "" {
		return fmt.Errorf("callback is nil")
	}
	// NOTE: transform_type
	if req.BideParams.TransformType == "" {
		return fmt.Errorf("transform_type is nil")
	}
...
func (h *Handler) MakeReq(req *types.ConvReq) (*HandlerReq, error) {
	// step1. url解码
	callbackURL, err := url.QueryUnescape(req.BideParams.Callback)
	if err != nil {
		return nil, fmt.Errorf("failed to unescape mgc_cb, err: %w", err)
	}
	// step2. 拼接事件
	callback := fmt.Sprintf("%s&transformType=%v", callbackURL, req.BideParams.TransformType)
